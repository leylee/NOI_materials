...
begin
 ...
 {   for j:=1 to i do
    begin
     if j=1 then if max(f[i,i],f[i,j+1])+a[i,j]<f[i,j] then
       f[i,j]:=max(f[i,i],f[i,j+1])+a[i,j];
     if j=i then if max(f[i,1],f[i,j-1])+a[i,j]<f[i,j] then
       f[i,j]:=max(f[i,1],f[i,j-1])+a[i,j];
     if (j<>1) and (j<>i) then if max(f[i,j-1],f[i,j+1])+a[i,j]<f[i,j] then
     f[i,j]:=max(f[i,j-1],f[i,j+1])+a[i,j];
    end;  }
      for j:=1 to i-1 do
